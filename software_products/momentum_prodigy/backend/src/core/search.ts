...
/** Search result category used for filtering and navigation decisions. */
export type SearchHitType = 'concept' | 'lesson' | 'deck' | 'card' | 'source' | 'practice';

/** Public search result shape consumed by the Search screen. */
...
/** Describes the IndexItem data shape so future readers know what fields travel through the app. */
interface IndexItem {
  type: SearchHitType;
  id: string;
  title: string;
...
/** Offline hybrid index combining keyword matches with precomputed semantic vectors. */
export class SearchIndex {
  ...
  /** Indexes every searchable course item after the course loads. */
  ...
  /** Refreshes the searchable practice-card entries after edits or generation. */
