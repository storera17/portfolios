...
//     results as grounding context. backend/scripts-fetch-models.mjs downloads
//     the recommended weights (Qwen2.5-3B-Instruct Q4 for desktop, 1.5B for
//     iPhone).
//
...
/** One message in the session-only tutor conversation. */
export interface TutorTurn {
  role: 'user' | 'tutor';
  text: string;
}

/** Optional local-model bridge; any implementation must generate fully offline. */
...
/** Describes the Retrieved data shape so future readers know what fields travel through the app. */
interface Retrieved {
  concept: Concept;
  score: number;
}

/** Common words ignored by lightweight keyword matching. */
...
/** Tokenizes text for simple keyword scoring. */
const tok = (s: string) => s.toLowerCase().split(/[^a-z0-9]+/).filter((w) => w.length > 2 && !STOP.has(w));

/** Offline tutor service that retrieves relevant course concepts and composes answers. */
export class Tutor {
  ...
  /** Returns an optional local LLM bridge when the host app provides one. */
  private bridge(): LLMBridge | null {
    return (typeof window !== 'undefined' && window.__MomentumProdigy_LLM) || null;
  }

  /** Finds the most relevant course concepts for a tutor question. */
...
/** Lowercases the first character when combining explanation fragments grammatically. */
